...
- **Accueil** : 🆕 Nouveautés (ajoutés depuis moins de 14 jours) → 🏷️ Promotions → 🏪 Tous les produits (grille) ; chaque produit n'apparaît qu'une fois.
- **Variantes** (taille, couleur, pointure… 5 groupes max) avec supplément de prix et **stock par valeur** (vide = non suivi ; valeur épuisée grisée, quantité limitée au stock, stock remis si la commande est annulée/retournée). Même éditeur de produit dans l'espace vendeur et dans l'admin (⚙️ Stock), où l'admin peut aussi choisir le vendeur ; **quantité − / +** et **total calculé en direct** dans le formulaire. Le serveur recalcule toujours le montant (prix + suppléments) × quantité ; la variante choisie figure sur la commande.
- **Description** : un lien YouTube / TikTok / Instagram / Facebook / Vimeo seul sur sa ligne affiche la vidéo ; les autres liens deviennent cliquables (réseaux sociaux avec leur icône). Page complète `/boutique?tout=1` avec filtres par catégorie, 🏷️ promos (`promo=1`) et tri (`tri=prix_asc|prix_desc|promo`).
